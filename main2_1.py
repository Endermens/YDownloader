...
class YDownloader:
    ...
    def __programm_interface(self):
        ...
        self.download_button = gui.Button(master = self.body, text='скачать', width=16, height= 2, font='Arial 18', bg="#FF6347", fg="#F8F8FF", command=lambda: threading.Thread(target=self.get_shref).start()).pack(side="left", anchor="e", expand=True, pady=5)
        self.var = gui.IntVar()
        self.var.set(1)
